...
with DAG(
    ...
    def get_jira_ticket_id(**kwargs):
        ...
        if "jira_ticket_id" in kwargs["dag_run"].conf.keys():
            ...
            assert isinstance(jira_ticket_id, list), (
                "Input 'jira_ticket_ids' object needs to be a list of the"
    ...
    def grab_issue_content(issue):
        ...
        if request_type == "Publish Dataset":
            ...
            issue_metadata = {
                ...
                if fields[mapping["dataset_category"]]["value"] == "Yes"
                else None,
    ...
    def get_all_jira_issues(**kwargs):
        ...
        jira_ticket_ids = kwargs.pop("ti").xcom_pull(task_ids="get_jira_ticket_id")[
            "jira_ticket_id"
        ]
        logging.info(f"---Jira Ticket Id----{jira_ticket_ids}-----------")
        ...
        for issue in issues:
            if issue["key"] in jira_ticket_ids:
                ...
                yaml_files_list[issue["key"]] = filename
            else:
                continue

        return {"generated-yaml-list": yaml_files_list}
    ...
    get_jira_ticket_id >> get_all_jira_issues >> slack_notificaiton
