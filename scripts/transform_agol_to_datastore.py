...
REMOVE_COLUMNS = ['x','y','lat','lon','latitude','longitude','shape_area','shape_length']
...
for column in columns['TIMESTAMP']:
    data[column] = data[column].apply(lambda x: datetime.utcfromtimestamp(x / 1000).strftime('%Y-%m-%d %H:%M:%S') if not pd.isnull(x) else x)
